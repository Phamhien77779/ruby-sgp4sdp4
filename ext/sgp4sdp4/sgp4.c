...
static VALUE calculate(VALUE self, VALUE since) {
  ...
  SGP4(NUM2DBL(since), &cTle, params, pos, vel);
...
static VALUE initialize(VALUE self, VALUE tle) {
  ...
  // puts("Got struct.");
  SGP4_init(params, &cTle);
  // puts("Past SGP_init.");

  return self;
}

void Init_sgp4sdp4_sgp4() {
  VALUE cSgp4sdp4_sgp4 = rb_define_class_under(mSgp4sdp4, "Sgp4", cSgp4sdp4_ephemeris);
  // rb_define_alloc_func(cSgp4sdp4_sgp4, allocate);
  rb_define_module_function(cSgp4sdp4_sgp4, "initialize", initialize, 1);
  rb_define_method(cSgp4sdp4_sgp4, "calculate", calculate, 1);
}
