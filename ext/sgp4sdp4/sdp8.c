...
static VALUE calculate(VALUE self, VALUE since) {
  ...
  SDP8(NUM2DBL(since), &cTle, params, pos, vel);
...
static VALUE initialize(VALUE self, VALUE tle) {
  ...
  // puts("Got struct.");
  SDP8_init(params, &cTle);
  // puts("Past SGP_init.");

  return self;
}

void Init_sgp4sdp4_sdp8() {
  VALUE cSgp4sdp4_sdp8 = rb_define_class_under(mSgp4sdp4, "Sdp8", cSgp4sdp4_ephemeris);
  // rb_define_alloc_func(cSgp4sdp4_sdp8, allocate);
  rb_define_module_function(cSgp4sdp4_sdp8, "initialize", initialize, 1);
  rb_define_method(cSgp4sdp4_sdp8, "calculate", calculate, 1);
}
